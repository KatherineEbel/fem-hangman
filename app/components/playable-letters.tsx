  ...
  return (
      ...
      >
        ...
        {!disabled ? (
            <>
              <div
                  className="absolute inset-0 -z-10 before:absolute before:h-1 before:w-1 before:animate-sparkle before:rounded-full before:bg-white before:opacity-0 after:absolute after:h-1 after:w-1 after:animate-sparkle2 after:rounded-full after:bg-white after:opacity-0"></div>
            </>
        ) : null}
      </div>
  )
  ...
  return (
      <div className="flex flex-wrap gap-4 justify-center items-center">
        {words.map((word, index) => (
            <div key={index} className={clsx(word.length <= 3 ? 'mx-8' : 'mx-28', 'flex gap-1 whitespace-nowrap')}>
